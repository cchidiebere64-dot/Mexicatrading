...
import {
  ...
  Eye, EyeOff, CheckCircle, AlertTriangle, ChevronDown, Search, Gift
} from "lucide-react";
...
/* ── Fade-in field wrapper ── */
const FadeField = ({ show, children }) => (
  <AnimatePresence>
    {show && (
      <motion.div
...
const Field = ({ icon: Icon, children }) => (
  <div className="relative group">
    {Icon && <Icon size={14} className="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none transition-colors duration-300 text-white/25 group-focus-within:text-emerald-400" />}
    {children}
  </div>
);
...
const ProgressDots = ({ total, current }) => (
  <div className="flex items-center gap-2 justify-center mb-8">
    {Array.from({ length: total }).map((_, i) => (
      <motion.div key={i}
        animate={{ width: i === current ? 24 : 6, opacity: i <= current ? 1 : 0.2, background: i < current ? "#10b981" : i === current ? "linear-gradient(90deg,#10b981,#14b8a6)" : "rgba(255,255,255,.2)" }}
        transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
        className="h-1.5 rounded-full"
        style={{ background: i < current ? "#10b981" : i === current ? "#10b981" : "rgba(255,255,255,.15)" }} />
    ))}
  </div>
);
...
export default function Register() {
  ...
  const [showConfirm_f,   setShowConfirm_f]   = useState(false);
  ...
  const scrollToBottom = () => setTimeout(() => bottomRef.current?.scrollIntoView({ behavior: "smooth", block: "end" }), 100);
  const focusAfter = (ref) => setTimeout(() => ref.current?.focus(), 600);

  const handleChange = (e) => setForm({ ...form, [e.target.name]: e.target.value });

  /* ── Field reveal triggers ── */
  const onNameBlur = () => {
    if (form.name.trim().length >= 2 && !showEmail) {
      setShowEmail(true);
      focusAfter(emailRef);
      scrollToBottom();
    }
  ...
  const onEmailBlur = () => {
    if (/\S+@\S+\.\S+/.test(form.email) && !showCountry) {
      setShowCountry(true);
      scrollToBottom();
    }
  };
  const onCountrySelect = (country) => {
    ...
    setCountrySearch("");
    setError("");
    if (!showPhone) { setShowPhone(true); focusAfter(phoneRef); scrollToBottom(); }
  };
  const onPhoneBlur = () => {
    if (form.phoneNumber.trim().length >= 5 && !showPassword_f) {
      setShowPassword_f(true);
      focusAfter(passwordRef);
      scrollToBottom();
    }
  };
  const onPasswordBlur = () => {
    if (form.password.length >= 6 && !showConfirm_f) {
      setShowConfirm_f(true);
      focusAfter(confirmRef);
      scrollToBottom();
    }
  };
  const onConfirmBlur = () => {
    if (form.confirmPassword.length >= 6 && !showReferral) {
      setShowReferral(true);
      scrollToBottom();
      setTimeout(() => { setShowTerms(true); scrollToBottom(); }, 400);
    }
  };

  /* ── Progress step calc ── */
  const step = showTerms ? 6 : showReferral ? 5 : showConfirm_f ? 4 : showPassword_f ? 3 : showPhone ? 2 : showCountry ? 1 : showEmail ? 0 : 0;
  ...
  const validatePhone = () => {
    ...
    if (!expected.includes(digits.length)) { const range = expected.length===1?`${expected[0]} digits`:`${expected.join(" or ")} digits`; return `Phone for ${selectedCountry.name} must be ${range}. You entered ${digits.length}.`; }
    return null;
  };
  ...
  return (
    <div className="relative flex flex-col items-center justify-start min-h-screen bg-[#080c18] text-white overflow-x-hidden px-4 py-12"
      ...
      <style>{`
        ...
        .country-row{transition:background .2s;}
        .country-row:hover{background:rgba(16,185,129,.08);}
        .country-row.active{background:rgba(16,185,129,.1);}
        /* Completed field style */
        ...
        <AnimatePresence mode="wait">
          ...
          {success ? (
            <motion.div key="success" initial={{opacity:0,scale:.96}} animate={{opacity:1,scale:1}} exit={{opacity:0}}
              ...
              <div>
                <h2 className="serif font-light mb-2 text-white" style={{fontSize:"clamp(28px,5vw,38px)"}}>Account Created!</h2>
            ...
            <div className="px-8 py-9" style={{background:"rgba(255,255,255,.025)",border:"1px solid rgba(255,255,255,.07)",backdropFilter:"blur(24px)"}}>
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-xs font-light tracking-wide" style={{color:"rgba(255,255,255,.35)"}}>
                  Fill in your details — each step unlocks the next.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit}>
                <div className="space-y-3">
                  ...
                  {/* ── 1. FULL NAME (always visible) ── */}
                  <Field icon={User}>
                    <input
                      type="text" name="name" value={form.name} onChange={handleChange}
                      onBlur={onNameBlur}
                      placeholder="Full Name"
                      className={`${inp} ${form.name.trim().length >= 2 ? "field-done" : ""}`}
                      autoFocus />
                    {form.name.trim().length >= 2 && (
                      <CheckCircle size={13} className="absolute right-4 top-1/2 -translate-y-1/2" style={{color:"var(--em)"}} />
                    )}
                  </Field>

                  {/* ── 2. EMAIL ── */}
                  <FadeField show={showEmail}>
                    <Field icon={Mail}>
                      <input
                        ref={emailRef}
                        ...
                        className={`${inp} ${/\S+@\S+\.\S+/.test(form.email) ? "field-done" : ""}`} />
                  ...
                  {/* ── 3. COUNTRY ── */}
                  <FadeField show={showCountry}>
                    <div className="relative" ref={countryRef}>
                      <button type="button" onClick={()=>setCountryOpen(!countryOpen)}
                        className={`w-full flex items-center justify-between px-4 py-4 border text-sm text-left transition-all duration-300 ${selectedCountry?"field-done":""}`}
                            ...
                            <div className="overflow-y-auto flex-1">
                              {COUNTRIES.filter(c=>c.name.toLowerCase().includes(countrySearch.toLowerCase())||c.dial.includes(countrySearch)).map(c=>(
                                <button key={c.code} type="button" onClick={()=>onCountrySelect(c)}
                                  className={`country-row w-full flex items-center gap-3 px-4 py-2.5 text-sm text-left ${selectedCountry?.code===c.code?"active":""}`}>
                      ...
                      <div className="flex gap-2">
                        ...
                        <Field icon={Phone}>
                          <input
                            ...
                            className={`${inp} ${form.phoneNumber.trim().length >= 5 ? "field-done" : ""}`} />
                  ...
                  {/* ── 5. PASSWORD ── */}
                  <FadeField show={showPassword_f}>
                    <Field icon={Lock}>
                      <input
                        ref={passwordRef}
                        type={showPassword?"text":"password"} name="password" value={form.password}
                        onChange={handleChange} onBlur={onPasswordBlur}
                        placeholder="Password (min 6 characters)"
                        className={`${inp} pr-11 ${form.password.length >= 6 ? "field-done" : ""}`} />
                  ...
                  {/* ── 6. CONFIRM PASSWORD ── */}
                  <FadeField show={showConfirm_f}>
                    <div>
                      <Field icon={Lock}>
                        <input
                          ref={confirmRef}
                          type={showConfirm?"text":"password"} name="confirmPassword" value={form.confirmPassword}
                          onChange={handleChange} onBlur={onConfirmBlur}
                          placeholder="Confirm Password"
                          className={`${inp} pr-11 ${form.confirmPassword.length >= 6 && form.password===form.confirmPassword ? "field-done" : ""}`} />
                  ...
                  {/* ── 7. REFERRAL (optional) ── */}
                  <FadeField show={showReferral && !refCode}>
                    <div className="relative">
                      <Gift size={14} className="absolute left-4 top-1/2 -translate-y-1/2 pointer-events-none" style={{color:"rgba(255,255,255,.25)"}} />
                      <input
                        ref={referralRef}

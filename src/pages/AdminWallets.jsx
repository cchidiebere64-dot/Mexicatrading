...
const [wallets, setWallets] = useState({}); // Object keyed by name
...
const walletId = Object.entries(wallets).find(([name]) => name === editName)[1]._id;
const res = await axios.put(`${API_URL}/api/admin/wallets/${walletId}`, form, {
headers: { Authorization: `Bearer ${token}` },
});
...
setForm({ name, address: w.address, caution: w.caution });
setEditName(name);
};
...
try {
const walletId = Object.entries(wallets).find(([n]) => n === name)[1]._id;
const res = await axios.delete(`${API_URL}/api/admin/wallets/${walletId}`, {
headers: { Authorization: `Bearer ${token}` },
});
alert(res.data.message);
fetchWallets();
...
```
    <form onSubmit={handleSubmit} className="mb-6 space-y-4 max-w-md">  
      ...
      >  
        {actionLoading ? "Processing..." : editName ? "Update Wallet" : "Add Wallet"}  
      </button>  
      {editName && (  
        <button  
          type="button"  
          className="ml-2 px-6 py-2 rounded-xl bg-red-500 text-white hover:bg-red-600"  
          onClick={() => { setForm({ name: "", address: "", caution: "" }); setEditName(""); }}  
        >  
          Cancel  
        </button>  
      )}  
      ...
      <table className="min-w-full border bg-white shadow-md">  
        ...
        <tbody>  
          {Object.entries(wallets).length === 0 ? (  
            <tr>  
              <td colSpan={4} className="text-center p-4">No wallets found</td>  
            </tr>  
          ) : (  
            Object.entries(wallets).map(([name, wallet]) => (  
              <tr key={name} className="border-b">  
                <td className="p-3 border">{name}</td>  
                <td className="p-3 border break-all">{wallet.address}</td>  
                <td className="p-3 border">{wallet.caution}</td>  

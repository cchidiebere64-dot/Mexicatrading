  ...
  return (
    <div className="p-6 min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-900 dark:text-white">
      ...
      <div className="mb-8 text-center">
        <h2 className="text-4xl font-bold text-emerald-600">👋 Welcome, {name}</h2>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-lg mb-8">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-4xl font-extrabold text-emerald-500 mt-2">${balance}</p>
          </div>
          ...
          {plans.length > 0 ? (
            plans.map((plan, idx) => (
              <div key={idx} className="border dark:border-gray-700 p-4 rounded-lg hover:shadow-md transition">
                ...
                <p className="text-emerald-500 font-semibold">Profit: ${plan.profit ?? 0}</p>
              </div>
            ))
          ) : (
          ...
          {history.length > 0 ? (
            history.map((item, idx) => (
              <li key={idx} className="flex justify-between items-center border-b dark:border-gray-700 pb-2">
                ...
                <span
                  className={`font-semibold ${item.action === "Deposit" ? "text-emerald-500" : "text-red-500"}`}
                >
                  ${item.amount ?? 0}
                </span>
              </li>

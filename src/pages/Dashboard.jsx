...
export default function Dashboard() {
  ...
  // TradingView chart - runs only after data is loaded
  useEffect(() => {
    if (!data) return;
      ...
      <main className="pt-20 px-4 max-w-5xl mx-auto animate-fade-in space-y-8">
        ...
        <section>
          ...
          {plans.length ? (
                    ...
                    <div className="relative w-16 h-16">
                      <svg className="w-16 h-16" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <circle
                          ...
                          cx="18" cy="18" r="15"
                          stroke="#a7f3d0" strokeWidth="2" fill="none"
                          strokeDasharray="12 90" strokeDashoffset="0"
                          strokeLinecap="round"
            ...
            <div className="flex flex-col items-center justify-center py-10 rounded-xl border border-white/10 bg-white/5 text-center gap-3">
              <TrendingUp size={36} className="text-emerald-400 opacity-50" />
              <p className="text-white font-semibold text-lg">No Active Plans Yet</p>
              <p className="text-white/40 text-sm max-w-xs">
                Start growing your wealth by choosing an investment plan tailored for you.
              </p>
              <button
                onClick={() => navigate("/plans")}
                className="mt-2 px-5 py-2 rounded-lg bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 text-sm hover:bg-emerald-500/30 transition-all"
              >
                Browse Plans →
              </button>
            </div>
        ...
        <section>
          ...
          {completed.length ? (
            ...
            <div className="flex flex-col items-center justify-center py-10 rounded-xl border border-white/10 bg-white/5 text-center gap-3">
              <ArrowUpCircle size={36} className="text-emerald-400 opacity-50" />
              <p className="text-white font-semibold text-lg">No Completed Plans Yet</p>
              <p className="text-white/40 text-sm max-w-xs">
                Your completed investments and earned profits will appear here.
              </p>
            </div>
          )}
        </section>
        ...
        <section>
          ...
          {history.length ? (
            <div className="space-y-2">
              {history.map((h, i) => (
                ...
                >
                  ...
                  <span className={h.action === "Deposit" ? "text-emerald-400" : "text-red-500"}>
                    ${h.amount ?? 0}
                  </span>
                </div>
              ))}
            ...
            <div className="flex flex-col items-center justify-center py-10 rounded-xl border border-white/10 bg-white/5 text-center gap-3">
              <Wallet size={36} className="text-emerald-400 opacity-50" />
              <p className="text-white font-semibold text-lg">No Transactions Yet</p>
              <p className="text-white/40 text-sm max-w-xs">
                Your deposits, withdrawals and activity history will show up here.
              </p>
              <button
